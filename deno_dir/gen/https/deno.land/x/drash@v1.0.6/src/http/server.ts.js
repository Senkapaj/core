...
let Server = /** @class */ (() => {
    class Server {
        ...
        async getRequest(request) {
            ...
            request = await new Drash.Services.HttpRequestService().hydrate(request, options);
            return request;
        }
        /**
         * @description
        ...
        async handleHttpRequest(request) {
            ...
            try {
                ...
                this.logDebug("Using `" + resource.constructor.name +
         ...
         *     See `Drash.Http.Response.send()`.
         */
        async handleHttpRequestError(request, error, resource = {}, response = {}) {
         ...
         *     testing purposes.
         */
        handleHttpRequestForFavicon(request) {
         ...
         *     testing purposes.
         */
        handleHttpRequestForStaticPathAsset(request) {
            try {
                const response = new Drash.Http.Response(request, {
                    views_path: this.configs.views_path,
                    template_engine: this.configs.template_engine,
                ...
                const contents = Deno.readFileSync(`${this.directory}/${request.url_path}/index.html`);
                if (contents == null) {
                    return response.sendStatic(`${this.directory}/${request.url_path}`);
                }
         ...
         */
        getResourceObject(resourceClass, request) {
            const resourceObj = new resourceClass(request, new Drash.Http.Response(request, {
                views_path: this.configs.views_path,
                template_engine: this.configs.template_engine,
            }), this);
         ...
         * @return void
         */
        async executeMiddlewareServerLevelBeforeRequest(request) {
         ...
         * @return void
         */
        async executeMiddlewareServerLevelAfterRequest(request, response) {
         ...
         */
        httpErrorResponse(code, message) {
        ...
        getResourceClass(request) {
            for (const resourceName in this.resources) {
                ...
                for (const pathObj of resource.paths) {
                    ...
                    const pathMatchesRequestPathname = request.url_path.match(pathObj.regex_path);
        ...
        requestTargetsStaticPath(request) {
            ...
            request = new Drash.Services.HttpRequestService().hydrate(request, {
                headers: {
                    "Response-Content-Type": new Drash.Services.HttpService().getMimeType(request.url, true),
                },
            });
            return true;
        }
    ...
    Server.REGEX_URI_MATCHES = new RegExp(/(:[^(/]+|{[^0-9][^}]*})/, "g");
    Server.REGEX_URI_REPLACEMENT = "([^/]+)";
    return Server;
})();
export { Server };
//# sourceMappingURL=file:///Users/marianhahne/Code/Senkapaj/core/deno_dir/gen/https/deno.land/x/drash@v1.0.6/src/http/server.ts.js.map